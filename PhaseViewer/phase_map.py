...
def phase_fig(data_wave, ref_model="ak135", filter_data=False, filter_freq=[1, 3], phase_name=["P","PcP","PKiKP"], wave_view_win=[0,1200], view_win=[-30,30], sta_win=[-10, 10], lta_win=[-30, -10], cross_win=[-10, 10]):
    ...
    for j in range(column_num):
        ...
        if P_wave_yes == 1:
            ax.plot(wave_cut_times['lta_cut'], phase_wave_info['lta_cut']['P'], color='silver')
            ax.plot(wave_cut_times['sta_cut'], phase_wave_info['sta_cut']['P'], color='green')
            ax.plot(wave_cut_times['view_cut'][int((sta_win[1]-view_win[0])*samplate):], phase_wave_info['view_cut']['P'][int((sta_win[1]-view_win[0])*samplate):], color='k')
            if np.abs(lta_win[0]) < np.abs(view_win[0]):
                ax.plot(wave_cut_times['view_cut'][:int((lta_win[0]-view_win[0])*samplate)], phase_wave_info['view_cut']['P'][:int((lta_win[0]-view_win[0])*samplate)], color='k')
            ax.text(view_win[0],ax.get_ylim()[0]*text_loc[0],'MEAN:%.2f'%mean_info['P'], color='green')
            ax.text(view_win[0],ax.get_ylim()[1]*text_loc[1],'SNR:%.2f'%SNR_info['P'], color='green')
            ax.fill_between([sta_win[0],sta_win[1]],ax.get_ylim()[0],ax.get_ylim()[1],facecolor = 'lightgreen', alpha = 0.3)
            ax.fill_between([lta_win[0],lta_win[1]],ax.get_ylim()[0],ax.get_ylim()[1],facecolor = 'lightgrey', alpha = 0.3)
        ax.set_xlim(view_win)
        ax.set_xlabel('Time (s)')
        ax.set_ylabel('Amplitude')
        ax.set_title('Phase: P')
    # row 3: PcP and PKiKP phases, row 4: cross correlation with P phase
    ...
    for i,phase in enumerate(phase_name):
        # PcP and PKiKP phases
        ax = plt.subplot(row_num, column_num, i+5)
        ax.plot(wave_cut_times['lta_cut'], phase_wave_info['lta_cut'][phase], color='silver')
        ax.plot(wave_cut_times['sta_cut'], phase_wave_info['sta_cut'][phase], color=phase_color[i])
        ...
        ax.set_xlabel('Time (s)')
        ax.set_ylabel('Amplitude')
        ax.set_title('Phase: %s'%phase)
        # cross correlation
        ax = plt.subplot(row_num, column_num, i+7)
        if P_wave_yes == 1:
